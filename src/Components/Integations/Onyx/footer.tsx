...
const FooterTitle = () => {
    ...
    return (
        <p className="text-[#E5E7EB] text-sm text-left">
            {t("Bhavish Finance")}
        </p>
    );
};
...
const OnyxFooter = () => {
    const { t } = useTranslation();
    return (
        <div className="px-4 bottom-4 mb-[150px] w-full sm:mb-24 max-w-[1376px] mlgh:box-content  mx-auto">
            <div className="border-t border-gray-400 pt-6 pb-10 flex items-start sm:items-center dark:border-gray-400 justify-between flex-col md:flex-row">
                <FooterTitle />
                    ...
                    <div className="w-full sm:w-fit justify-around flex space-x-6">
                        {React.Children.toArray(
                            footerInfo.map(info => (
                                <a
                                    target="_blank"
                                    className="text-[#217BF4] text-left text-sm"
                                    href={info.url}
                                    rel="noreferrer"
                                >
                                    {t(info.name)}
